...
Modify <<program-ticker-two-flash>> by creating a timer-has-stopped callback that turns off the fast LED in the event that the fast timer is stopped and prints a log message about being stopped.
...
. You will now look at the variability of this behavior.  Repeat the btn:[Single] acquisition 4 more times and pay attention to the LED "`on`" time.  Do you get 2.0000 s each time?
