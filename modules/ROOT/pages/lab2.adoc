...
----
. Select btn:[Add build configuration panel] through the nRF Connect side bar and select our board as the target. In a change from past procedure, check the **Generate only** box. This will change the final button to btn:[Generate]. Click on this button.
+
[#img-generate-config-not-build]
.Generate the configuration but do not build the application.
image::lab2/nrf-connect-generate-2026.png[Generate only,542,285]
+
. In the **APPLICATIONS** section of the nRF Connect side panel, select the entry for your particular build configuration under the **build** subsection (it should be named something like `external-leds - nRF52840 DK nRF52840`).
. In the **EXTERNAL-LEDS** section of the nRF Connect side panel, expand the **Config files** entry and then the **Devicetree** entry below that. You should see an item labeled `nrf52840dk_nrf52840.dts` and below that a **+ Create overlay file** item. Click on the **Create overlay** item.
. If you look in the files for your application, you should now see a new file named `nrf52840dk_nrf52840.overlay`. Add the following to that file.
+
[source, dts]
...
. Access the GitHub Classroom assignment link (found on Canvas) and create the repository for this assignment.
+
