...
1. Download the https://www.nordicsemi.com/Products/Development-tools/nRF-Command-Line-Tools/Download[nRF Command Line Tools] from Nordic Semiconductor.
2. Install the tools. This also includes a separate installer for the required SEGGER J-Link software.
...
You can install by following the links above or from within VS Code using the Extensions panel. You open this by clicking on the Extensions icon in the Activity bar.
...
From the Extensions panel you can search for the extensions listed above and then install from their page within VS Code.
