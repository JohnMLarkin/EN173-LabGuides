...
Before you can begin writing and running code on your microcontroller development board, you need to install several software tools. These include:

=== SEGGER J-Link

. Download the SEGGER J-Link software from https://www.segger.com/downloads/jlink[SEGGER]. These instructions have been tested using v8.96. 
...
+
[#img-segger-jlink-install-legacy-drivers]
.SEGGER J-Link 'Choose Optional Components' dialog.
image::lab1/segger-j-link-install.png[J-Link Install, 644, 382]
+
...
.. Once all selections have been made, it triggers a substantial download and install process so be prepared to wait for ten or more minutes.
. Click on btn:[Manage SDKs] and then select the btn:[Install SDK] action.
.. These guides were tested using SDK {nrf-sdk}. You want a version that matches that of the toolchain you just installed.
...
+
. Next, set the board target to match your microcontroller. If you are using the nRF52840 DK board you should select `nrf52840dk/nrf52840` (this was `nrf52840dk_nrf52840` in older versions of the SDK, as shown in the image below).
+
[#img-set-board-target]
.Add a build configuration from the nRF Connect side bar.
image::lab1/nrf-connect-set-board-target.png[Set board target, 541, 198]
+
. Finish by clicking the btn:[Generate and Build] button. The initial generation of a configuration and building of an application may take several minutes.
...
. You need to begin by accessing the GitHub Classroom assignment link (as found on Canvas).
+
