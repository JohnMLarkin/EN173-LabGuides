...
. You will need to add this folder to your system's PATH variable. This is done by opening the System Properties dialog box, selecting the Advanced tab, and then clicking on the Environment Variables button. In the System variables section, find the PATH variable and click on the Edit button. Add the path to the folder where `nrfutil` is located.
...
// Verified with SDK 2.9.0 and nRF52840 DK on 3/4/2025
[source,c]
