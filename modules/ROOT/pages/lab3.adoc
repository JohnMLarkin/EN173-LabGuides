...
+
. Select btn:[Add build configuration panel] through the nRF Connect side bar and select our board as the target. Check the **Generate only** box so the final button becomes btn:[Generate]. Click on this button.
. In the **APPLICATIONS** section of the nRF Connect side panel, select the entry for your particular build configuration under the **build** subsection
. In the section of the nRF Connect side panel for your particular build configuration, expand **Devicetree** entry (under **Config files**) to reveal **+ Create overlay file** option. Click on this.
...
----
<1> The node label has been changed to `pb`, short for push button. You will need to change your code in `main.c` accordingly.
...
. Access the GitHub Classroom link for this assignment on Canvas and create a repository for your work.
+
...
IMPORTANT: When your program and circuit are working successfully, remember to push the commits to the remote repository. Also, take a video of its successful operation and upload this to Canvas.
====
