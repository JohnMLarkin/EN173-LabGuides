...
----
&i2c0 {
  compatible = "nordic,nrf-twim"; // <1>
  tempsensor: tmp102@48 { // <2>
...
----
<1> In nRF SDKs v3.2.0 and later this line is required. Earlier versions of the SDK do not need it.
<2> The device name is `tmp102` and the address is 0x48.
<3> The address is 0x48.
+
...
. Access the assignment link on Canvas.
...
IMPORTANT: Take a video demonstrating successful completion of this project and upload that to Canvas. Also, don't forget to push your final code to the remote repository.
...
. Access the GitHub Classroom link for this assignment on Canvas.
. Follow the usual steps for getting started with a repository from GitHub Classroom.
. When using the ADXL345 accelerometer with I^2^C communication, it must be wired differently than when using SPI.  The connections between the ADXL345 and the nRF52840 are given in <<table-ADXL345-I2C>>.
...
IMPORTANT: When your program and circuit are working successfully, remember to push the commits to the remote repository. Also, take a video of its successful operation (along with your reflection) and upload this to Canvas.

====

.ADXL345 to nRF52840 DK connections for I^2^C
[[table-ADXL345-I2C]]
|===
| Signal | nRF52840 pin
...
| GND | GND (via ground bus)
| VCC | VDD (via power bus)
| CS | VDD (via power bus)
| INT1 | no connection required
| INT2 | p0.04
| SDO | GND (via ground bus)
| SDA | p0.26 **and** 4.7 k{Omega} pull-up resistor to VDD
| SCL | p0.27 **and** 4.7 k{Omega} pull-up resistor to VDD
|===